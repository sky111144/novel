<template>
  <div class="wrapper">
    ...
    <div class="novel">
      ...
      <div class="section">
        <ul class="charpts">
          <li class="charpt" v-for="(charpt, index) in charpts">
...
export default {
  ...
  methods: {
    requestCharpts () {
      let nextPage = this.charpts.length / this.pageSize + 1
      ...
      let api = `${this.target}/novel/${this.novelId}?page=${nextPage}&size=${this.pageSize}`
      this.$http.get(api)
      .then((res) => {
    ...
    pullRequest () {
      ...
      let request = throttle(() => {
        ...
        // 判断是否下拉到底
        let currentPosition = document.documentElement.scrollTop || document.body.scrollTop || window.pageYOffset
        let targetPosition = document.body.clientHeight - window.innerHeight - 50
